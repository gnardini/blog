...
export function mapReferer(referrer?: string) {
  ...
  if (shortened.startsWith("http://"))
    shortened = shortened.substring("http://".length);
  if (shortened.startsWith("https://"))
    shortened = shortened.substring("https://".length);
  if (shortened.startsWith("www."))
    shortened = shortened.substring("www.".length);
  if (shortened.startsWith("old."))
    shortened = shortened.substring("old.".length);
  if (shortened.startsWith("masterychart.com")) return "masterychart.com";
  if (shortened.startsWith("google.") && shortened.length <= 15)
    return "google.com";
  if (shortened.startsWith("gnardini.com/"))
    return shortened.substring("gnardini.com".length);
  return shortened;
}
